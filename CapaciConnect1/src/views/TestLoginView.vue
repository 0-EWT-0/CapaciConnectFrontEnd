...
const uploadFile = async () => {
  ...
  try {
    const response = await genericRequestAuth(
      'https://localhost:44368/api/Multimedia/CreateMultimedia',
      'POST',
      multimediaDTO,
    )
...
const fetchMultimedia = async () => {
  try {
    const response = await genericRequestAuth(
      'https://localhost:44368/api/Multimedia/AllMultimedia',
      'GET',
    )
  ...
  if (confirm.isConfirmed) {
    try {
      await genericRequestAuth(
        `https://localhost:44368/api/Multimedia/DeleteMultimedia/${id_multimedia}`,
        'DELETE',
      )
      fetchMultimedia()
